            ...
            switch (source.customID) {
                ...
                case 'Barrier':
                    if (target.customID !== 'Event' &&
                        ...
                        target.customID !== 'Barrier') {
                        return 'A ' + source.customID + ' element can only connect to the following elements: Event, Consequence, or itself';
                    }
                    break;
                case 'Event':
                ...
                case 'Consequence':
                    if (target.customID !== 'Impact' &&
                        ...
                        return 'A ' + source.customID + ' element can only connect to the following elements: Impact Indicator, Barrier';
                    }
                    break;
...
EditorUi.prototype.generatePDF = function () {
    ...
    graphThreats.sort(function(threat1, threat2){
        ...
        else if(threat1.allDefined() && !threat2.allDefined()) return -1;
        else if(threat1.allDefined() && threat2.allDefined()){
            return (threat1.getProbability() >= threat2.getProbability() ? -1 : 1)
        } else return 0;
    });

    //This will sort all threats of the diagram, regarding their probability
    graphConsequences.sort(function(cons1, cons2){
        if(!cons1.allDefined() && cons2.allDefined()) return 1;
        else if(cons1.allDefined() && !cons2.allDefined()) return -1;
        else if(cons1.allDefined() && cons2.allDefined()){
            return (cons1.getProbability() >= cons2.getProbability() ? -1 : 1)
        } else return 0;
    });
    ...
    //Handle the case where getAttribute("infoDesc") return null
     function getDescription(element, index,editor){
         ...
         try{
             ...
             alert("Error while generating PDF. Check the link between elements \n" + "(note: the main cause of this error is when an element is unlink)");
         }
    }
    ...
    let docDefinition = {
        ...
        content: [{
            ...
            '\nThis document has been automatically generated by the BowTie++ application.',
            'We therefore propose the analysis of the main threats and consequences to be studied.', {
                text: '\n I - Bowtie diagram',
                style: 'subheader',
                color: '#00008B'
            }, {
            ...
            'This section presents an analysis of the main threats.\n',
            '\n', {
                style: 'tableExample',
                table: {
                    widths: [100, 60, '*'],
                    body: [
                        ...
                        [graphThreats[0] == null? 'No threat' : getDescription('threat',0,this.editor)[0], graphThreats[0] == null? '' : graphThreats[0].getProbability().toFixed(2), graphThreats[0] == null? '' : getDescription('threat',0,this.editor)[1]],
                        [graphThreats[1] == null? 'No threat' : getDescription('threat',1,this.editor)[0], graphThreats[1] == null? '' : graphThreats[1].getProbability().toFixed(2), graphThreats[1] == null? '' : getDescription('threat',1, this.editor)[1]],
                        [graphThreats[2] == null? 'No threat' : getDescription('threat',2,this.editor)[0], graphThreats[2] == null? '' : graphThreats[2].getProbability().toFixed(2), graphThreats[2] == null? '' : getDescription('threat',2, this.editor)[1]]
                    ]
                }
            }, {
                text: 'Detail of the main causes :',
                bold: true
            },
            'We remind the parameters of the likelihood matrix :', {
                bold: true,
                ul: [
                    'ACT = Actors : the weight indicates how large a group the actor represents in comparison to the other actors.',
                    'OPP = Opportunity : the weight should be based on the threat actor’s spatial, temporal and vulnerability exploiting opportunities.',
                    'MEA = Means : the weight should consider to what extent the different threat actors have the required means needed to perform the attack.',
                    'MTV = Motivation : the weight should be based on what motivation factors and intents that can be associated to each threat actor.'
                ]
            },
            '\n', {
                style: 'tableExample',
                table: {
                    ...
                    body: [
                        ...
                        [graphThreats[0] == null? 'No threat' : getDescription('threat',0,this.editor)[0], graphThreats[0] == null? '' : graphThreats[0].threatActors, graphThreats[0] == null? '' : graphThreats[0].opportunity, graphThreats[0] == null? '' : graphThreats[0].means, graphThreats[0] == null? '' : graphThreats[0].motivation, graphThreats[0] == null? '' : graphThreats[0].getProbability().toFixed(2)],
                        [graphThreats[1] == null? 'No threat' : getDescription('threat',1,this.editor)[0], graphThreats[1] == null? '' : graphThreats[1].threatActors, graphThreats[1] == null? '' : graphThreats[1].opportunity, graphThreats[1] == null? '' : graphThreats[1].means, graphThreats[1] == null? '' : graphThreats[1].motivation, graphThreats[1] == null? '' : graphThreats[1].getProbability().toFixed(2)],
                        [graphThreats[2] == null? 'No threat' : getDescription('threat',2,this.editor)[0], graphThreats[2] == null? '' : graphThreats[2].threatActors, graphThreats[2] == null? '' : graphThreats[2].opportunity, graphThreats[2] == null? '' : graphThreats[2].means, graphThreats[2] == null? '' : graphThreats[2].motivation, graphThreats[2] == null? '' : graphThreats[2].getProbability().toFixed(2)]
                    ]
                }
            },
            ...
            {
                text: '\n III - Main consequences',
                style: 'subheader',
                color: '#00008B'
            },
            'This section presents an analysis of the main consequences.\n',
            '\n', {
                style: 'tableExample',
                table: {
                    widths: [100, 50, '*'],
                    body: [
                        ...
                        [graphConsequences[0] == null? 'No consequence' : getDescription('consequence',0,this.editor)[0], graphConsequences[0] == null? '' : graphConsequences[0].getProbability().toFixed(2),graphConsequences[0] == null? '' : getDescription('consequence',0, this.editor)[1]],
                        [graphConsequences[1] == null? 'No consequence' : getDescription('consequence',0,this.editor)[0], graphConsequences[1] == null? '' : graphConsequences[1].getProbability().toFixed(2), graphConsequences[1] == null? '' : getDescription('consequence',1, this.editor)[1]],
                        [graphConsequences[2] == null? 'No consequence' : getDescription('consequence',0,this.editor)[0], graphConsequences[2] == null? '' : graphConsequences[2].getProbability().toFixed(2), graphConsequences[2] == null? '' : getDescription('consequence',2, this.editor)[1]]
                    ]
                }
            }, {
                text: 'Detail of the main consequences :',
                bold: true
            },
            'We remind the parameters of the impact matrix :', {
                bold: true,
                ul: [
                    'COM = Commercial : severity of the consequence on the commercial activity of the organisation that owns the system',
                    'REP = Reputation : severity of the consequence on the reputation of the organisation that owns the system',
                    'ENV = Environment : severity of the consequence on the environment',
                    'IND = Individual : severity of the consequence on the individuals around the system'
                ]
            },
            '\n', {
                style: 'tableExample',
            ...
            'Methods used for calculating the final risk :',
            {
                bold: false,
                ul: [
                    'p(U) :   The probability that the unwanted event U occurs',
                    'm :   The number of consequences',
                    'Cj :   The consequence j',
                    'p(Cj) :   The probability that the consequence j occurs',
                    'LCj :   The impact value of the consequence j',
                    'R(U)j :   Risk value for a consequence j'
                ]
            },
            {
                text : '\n Formulas : ',
